...
(define (start initial-request)
  (local [(define answer1 (get-answer (list-ref questions 0) (list-ref option-lists 0)))
          ...
          (define answer4 (if (string=? answer3 "Horizontal")
                              ...
                              ""))
          (define answer5 (if (string=? answer3 "Vertical")
          ...
          (define answer6 (if (string=? answer5 "Cave")
                              ...
                              ""))
          (define answer7 (if (string=? answer5 "Treasure")
...
(define (get-answer prompt options)
  (local [(define req 
          ...
          (define value (if (empty? bindings)
                            (error 'get-answer "User did not make a selection")
                            (if (and (exists-binding? 'customtxt bindings)
                                 (not (string=? "" (extract-binding/single 'customtxt bindings)))
                                 (string=? "Other" (extract-binding/single 'answer bindings)))
                            (extract-binding/single 'customtxt bindings)
